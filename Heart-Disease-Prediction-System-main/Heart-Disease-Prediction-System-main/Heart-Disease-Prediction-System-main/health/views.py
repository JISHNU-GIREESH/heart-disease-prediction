...
def add_heartdetail(request):
    if request.method == "POST":
        ...
        sorted_keys = sorted([k for k in post_data.keys() if k != 'csrfmiddlewaretoken'])
        
        # We need to maintain the order of features required by the model:
        # ['age','sex','cp', 'trestbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal']
        features = ['age','sex','cp', 'trestbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal']
        
        for feature in features:
            val = post_data.get(feature, '0')
            if feature == "sex":
                if val.lower() in ['male', 'm', '0']:
                    list_data.append(0)
                else:
                    list_data.append(1)
                continue
            try:
                list_data.append(float(val))
            except:
